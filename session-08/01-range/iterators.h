...
// Iterator (lat: "step-wise walker", "strider", "wanderer") [m], -oris
//
...
bool operator==(Iterator && it, const lazy_sequence_sentinel<T> & b)
{
  return b == it;
}

for (auto it = std::begin(range); not (it.==(std::end(range))); ++it) {
